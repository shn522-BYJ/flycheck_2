...
with tabs[0]:
    ...
    # 2) 标称几何覆盖与疑似缺口
    st.markdown("#### 🌾 标称覆盖：田块内有没有没飞到的地方？")
    if "错误" in cov:
        st.info(f"本架次标称覆盖无法计算：{cov['错误']}。")
    else:
        _n = cov.get("疑似几何缺口区域数量")
        if _n is None:
            st.warning("未提供田块空间边界，无法判断田块内有没有没飞到的地方。")
        elif _n == 0:
            st.success("按任务预设喷幅测算，田块内没有发现明显未飞到的区域。")
            st.caption("该结论不代表实际雾滴沉积或防治效果合格。")
        else:
            st.warning(
                f"按任务预设喷幅测算，有 {_n} 处未飞到的区域（疑似漏喷），"
                f"合计 {cov.get('疑似几何缺口面积_m2', 0):.2f} m²。"
                "“未飞到”指飞机航迹未经过该处，不等于一定没打上药，请现场核实。")
...
with tabs[2]:
    ...
    if "错误" in cov:
        ...
        m[2].metric("田块内未飞到区域", "未计算" if _gap_n is None else f"{_gap_n} 处")
        m[3].metric("标称重叠比例", f"{cov['重叠比例']}%")

        st.info(
            "本页计算的是**任务预设喷幅下的标称几何覆盖**。"
            "只有上传田块空间边界后，才能用‘田块−标称覆盖’标出**未飞到的区域**并估算面积；"
            "“未飞到”指飞机航迹未经过该处，只是**疑似漏喷**，不代表实际雾滴沉积或防治效果。"
        )
        st.caption(
        ...
        if x is not None:
            ...
            _add_polygons(cov.get("疑似缺口_local"), "未飞到区域（疑似漏喷）",
        ...
        if _gap_n is None:
            st.warning("未提供田块边界：无法判断田块内有没有没飞到的地方，也不能估算其面积。")
        elif gaps:
            st.warning(
                f"发现 {len(gaps)} 处未飞到的区域（疑似漏喷），"
                f"合计 {cov.get('疑似几何缺口面积_m2', 0):.2f} m²。"
                "“未飞到”指飞机航迹未经过，请结合现场核实是否实际漏喷。")
            ...
            st.success("在当前田块边界和任务预设喷幅口径下，没有发现未飞到的区域。该结果不证明实际沉积或防治效果合格。")
